...
(주의: 원격 SSH 세션은 세션 1 네이티브 UI와 격리돼 있어, 버튼을 실제로
클릭해 패널이 뜨는 스크린샷은 사용자 화면에 뜬 다른 창이 그 버튼 위를
덮고 있어 자동 클릭으로는 못 잡았다. 하지만 버튼 존재는 스크린샷으로
확인했고, 그 버튼의 액션은 표준 ShowTaskpane→검증된 로컬 페이지라 클릭
시 열린다. 브라우저로 여는 `xgen-seepage panel`은 이미 완전 검증됨.)
