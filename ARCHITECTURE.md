...
**막힌 지점(코드 문제 아님, 이 머신의 Office 환경 문제)**: 그 매니페스트를
실제 Excel 리본에 버튼으로 띄우는 마지막 단계가 안 됐다. 이 머신 Office가
**ProPlus2021 볼륨 라이선스, 빌드 16.0.14334**인데, 두 사이드로드 경로가
모두 막혔다: ① 개발자 레지스트리 사이드로드(`HKCU\...\WEF\Developer`)는
이 볼륨 빌드가 아예 안 읽는다(RuntimeLogging을 켜도 WEF 로그가 한 줄도
안 생김 = 애드인 로딩 시도 자체를 안 함). ② 네트워크 공유 폴더 카탈로그는
`삽입 → 내 추가 기능 → 모두 보기`로 뜨는 Office 애드인 삽입 대화상자가
WebView2 안에서 콘텐츠를 못 그려(검은/빈 화면) 공유 폴더 탭에 도달을 못
한다. `office.js` CDN은 200으로 되는데 삽입 대화상자 콘텐츠만 안 뜨는 걸로
봐서, 이 네트워크에서 애드인 삽입 서비스 엔드포인트가 막혔거나 이 구형
볼륨 빌드의 대화상자 렌더링 문제로 보인다. 원격(SSH) 세션은 세션 1의
네이티브 UI와 격리돼 있어 대화상자 클릭 자동화로도 못 넘겼다(예약 작업으로
세션 1에 키 입력·스크린샷까지 자동화해 여기까지 규명했지만, 대화상자
콘텐츠 자체가 안 그려지니 클릭할 대상이 없다). **결론: xgen-seepage
쪽은 전부 정상. 남은 건 순전히 그 데스크톱 Office 클라이언트가 웹 애드인을
표면에 못 올리는 환경 이슈다.** 최신 채널 Office(M365) 또는 애드인 삽입
서비스에 접근 가능한 네트워크에서 재시도하면 리본 버튼이 떠야 한다.
