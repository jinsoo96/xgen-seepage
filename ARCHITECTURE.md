...
**증명 안 된 것 - 실행 자체가 계속 `ERROR401`로 실패한다**: 워크플로우를
바꿔가며(하네스 전용 API로 생성 / §8과 완전히 같은 레거시 `/api/agentflow/
save` + `provider: anthropic, model: claude-haiku-4-5-20251001` 명시 조합)
여러 번 재현해봐도 실행이 전부 `{"type":"error","detail":"[ERROR401:
워크플로우 실행 중 오류가 발생했습니다...]"}`로 끝난다. 백엔드 소스
(`execution_core.py:1038`)를 확인해보니 ERROR401은 스트리밍 실행 중 발생하는
**모든 예외를 뭉뚱그리는 catch-all**이라 실제 원인(vLLM 모델 미기동인지,
워크플로우 로딩 문제인지, 다른 무엇인지)이 클라이언트로는 전혀 안 새어나온다.
같은 조사 중에 별개로 진짜 이상한 것도 하나 발견했다: 하네스 워크플로우를
`/api/agentflow/harness/workflows`(POST)로 만들면 `{"created": true}`가
돌아오는데, 그 직후 같은 API의 GET/LIST/DELETE 어디로도 안 보인다(레거시
`/api/agentflow/save`로 만든 것도 마찬가지). 이게 ERROR401의 원인인지,
아니면 이 공유 dev 서버가 그날 일시적으로 불안정했던 것인지(§8 워크로그에도
"공유 dev 환경 특성"으로 502/WS 재끊김이 있었다는 기록이 있다) 클라이언트
쪽에서는 구분할 방법이 없다 - 서버 로그 접근이 없어서 여기서 막힌다.

**Phase 2는 프록시 메커니즘만 실측 완료로 처리한다.** 실제 에이전트 응답이
끝까지 오는 전체 왕복(§8이 14개 도구 카탈로그로 했던 것과 같은 수준)은
dev 서버가 정상 상태일 때 재확인이 필요 - 다음에 이어갈 때 최우선 순위.

**남은 단계**: Phase 3(채팅 워크플로우 자동생성 - 위 실행 불안정 문제부터
먼저 풀어야 의미가 있다) → Phase 4(실제 Excel에 태스크팬 - **Excel이 없는
이 머신에서는 구조적으로 진행 불가**, §11 하단의 Excel COM 미검증 항목과
같은 성격. 게다가 Office.js 자체가 인터넷 전혀 없이 로드되는지도 MS 공식
문서로도 불확실해서(자체 호스팅 비권장, 실제 시도자는 인터넷 끄니 안
됐다고 보고) 실제 Excel 머신에서 함께 확인해야 함) → Phase 5(패키징).
