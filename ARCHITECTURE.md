...
폐쇄망 요구("인터넷 의존 아니라 XGEN 로그인이면 연결")에 맞춰, 남아있던
유일한 인터넷 의존이던 태스크팬의 `office.js`를 Microsoft CDN이 아니라
로컬 서버에서 서빙하도록 벤더링했다(`taskpane/vendor/office.js`). Playwright
네트워크 로그로 실측: 페이지 로드 시 office.js 요청이 로컬 하나뿐이고
**microsoft.com 호출이 0건**이다. **주의**: 벤더링한 건 office.js 부트스트랩
한 파일이고, 순수 브라우저에서는 Office 호스트가 없어 `Office.onReady()`가
완료를 못 했다(부트스트랩이 런타임을 호스트에서 받아야 하는데 브라우저엔
호스트가 없음). 실제 Excel 안에서는 Office 호스트가 런타임을 제공하므로
떠야 하지만, office.js가 런타임에 CDN에서 추가 리소스를 더 당기지는 않는지
포함해 **완전 에어갭 동작은 실제 폐쇄망 Excel에서 별도 확인이 필요**하다
(위 리본 버튼 미검증과 같은, 실제 Excel 머신이 있어야 푸는 항목).

**다만 핵심은 이거다**: 에이전트가 Excel에 침투해 셀을 읽고/쓰는 **본체
경로(로그인 → WS 브릿지 → tools → live_adapter/libreoffice_adapter)는
office.js도, 인터넷도 전혀 필요 없다.** XGEN 서버가 폐쇄망 안에 있으면 그
경로의 모든 통신이 폐쇄망 안에서만 오간다(§4·§5). office.js는 "Excel 안에서
채팅하는 태스크팬 UI"에만 필요했고, 그 UI를 안 띄우고 XGEN 웹에서 에이전트를
돌려도 셀 편집은 그대로 된다. 즉 폐쇄망에서 office.js가 끝내 문제가 되더라도
제품의 핵심 가치(에이전트의 Excel 자유 편집)는 인터넷 0으로 이미 동작한다.
