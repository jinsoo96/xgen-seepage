...
## 10. Excel 안 채팅 패널 (진행 중, 2026-08-17)

지금까지는 "채팅은 XGEN 어디서든(웹 UI 등), 편집은 로컬 Excel에 자동
반영"이었다. 사용자가 한 단계 더 나아가 **Excel 안에 실제 채팅 패널을
띄우는 것**을 요청했다.

**중요한 설계 정정(2026-08-17 사용자 지적)**: 처음엔 이걸 Office.js 태스크팬
애드인으로 접근했는데(§1-§2에서 뺐던 경로), 그게 틀렸다. 이 채팅 패널은
**Office JavaScript API를 하나도 쓰지 않는다** - 셀 편집은 이 페이지가
아니라 커넥터의 WS 브릿지(tools → live_adapter/libreoffice_adapter)가 하기
때문이다. 그래서 office.js에 의존할 이유가 애초에 없었다. 처음엔 Office
애드인 관례로 `Office.onReady()`를 넣었는데, 그게 오히려 Office 호스트를
기다리며 "Office.js 로딩중"에서 멈추게 만들었다(사용자가 이걸 지적).
office.js를 통째로 걷어내고, **XGEN 커넥터 연결(로컬 `/health`·`/workflows`·
`/chat/stream`)에만 의존하는 순수 웹 UI**로 바꿨다. 실측(Playwright): 페이지가
바로 뜨고("XGEN 연결됨"), 외부(Microsoft 포함) 요청이 0건이며, 에이전트
드롭다운·채팅 전송이 정상 동작한다. 이제 이 패널은 Excel 태스크팬으로
띄우든 그냥 브라우저로 열든 XGEN에만 붙어 있으면 동작한다 - Microsoft
애드인 인프라(office.js, 리본 등록)에 얽매이지 않는다.
...
얼린 exe로의 태스크팬 전체 왕복 검증이 그 내용이다. **Phase 4(실제 Excel에
태스크팬)는 부분적으로만 검증됐다 - 자세한 건 §11 후반부.** 태스크팬 웹
자산(manifest.xml, index.html, taskpane.js, taskpane.css)을 만들어
`taskpane/`에 넣었고, Playwright로 실제 브라우저에서 Office.js가 로드되고
`Office.onReady()`가 콜백되며 채팅 입력→전송→실제 SSE 스트리밍까지 도는 걸
확인했다. 다만 그 매니페스트가 실제 Excel 리본에 버튼으로 뜨는 마지막
단계는 이 검증에 쓴 머신의 Office가 웹 애드인 삽입 UI를 못 그려서 막혔다
(§11 후반). "워크플로우 자동생성"이던 옛 Phase 3은 폐기 - 연결 메커니즘으로
이미 대체됐다.
...
**같은 머신에서 Phase 4(실제 Excel 태스크팬)도 최대한 밀어붙였고, 어디서
막히는지 정확히 규명했다.** 태스크팬 웹 자산 4종(manifest.xml/index.html/
taskpane.js/taskpane.css)을 만들어, SSH 포트포워딩 + Playwright로 실제
브라우저에서 열어 검증했다: Office.js가 실제로 로드되고 `Office.onReady()`가
콜백되며(status가 "준비됨"으로 바뀜), 채팅창에 메시지를 입력·전송하면
로컬 `/chat/stream` 프록시 → 실제 dev-xgen.x2bee.com → SSE가 화면에
스트리밍되는 것까지 실제로 돌았다(응답 자체는 §8에서 알려진 vLLM 미기동
503만 남음). 즉 **태스크팬 UI·프록시·연결 전 경로는 실기로 증명됐다.**

**Excel 리본 버튼 - 폐쇄망에서 실제로 띄우는 데 성공(2026-08-17, 처음엔
헛다리 짚었다가 해결)**: 처음엔 "이 Office 2021 볼륨 빌드가 웹 애드인을
못 띄운다, M365 써야 한다"고 결론냈는데 **틀렸다** - 사용자가 정확히
반박했다: "M365는 폐쇄망이면 어차피 안 되잖아"(맞다 - 클라우드 구독이라
인터넷 없으면 애드인 삽입도 못 함). 그래서 클라우드를 안 거치는 로컬 사이드
로드를 제대로 파고들었다. 두 경로:
- ① 네트워크 공유 폴더 카탈로그 - 삽입 대화상자가 Microsoft 클라우드 UI라
  폐쇄망/구형 빌드에서 콘텐츠를 못 그린다(검은/빈 화면). **폐쇄망 부적합.**
- ② 개발자 레지스트리 사이드로드(`HKCU\...\WEF\Developer`) - **완전히
  로컬, 클라우드 무관.** 처음엔 안 됐는데(WEF 로그 안 생김) **원인은 내가
  레지스트리 값 이름을 틀리게 넣은 것**이었다: MS 문서상 값 **이름도
  매니페스트 전체 경로**여야 하는데(`이름=경로, 데이터=경로`) 나는 이름을
  "xgen-seepage"로 넣었었다. 이름을 경로로 고치니 **Excel 홈 탭 오른쪽에
  "XGEN > xgen-seepage 채팅" 버튼이 실제로 떴다**(스크린샷 확인). 즉
  폐쇄망에서 리본 버튼을 띄우는 로컬 경로가 실재하고 실측됐다.

**그걸 한 명령으로 자동화 - `xgen-seepage install-excel-addin`**: 손으로
레지스트리 편집할 필요 없이, (1) 인증서 생성+신뢰(WebView2가 로컬 HTTPS
페이지를 경고 없이 로드하도록), (2) 매니페스트를 실제 포트로 패치해 로컬
설치, (3) `WEF\Developer`에 올바른 형식(이름=경로)으로 등록까지 한 번에.
실측: 이 명령을 실행하니 레지스트리에 정확히 등록됐고(수동으로 넣어 버튼이
떴던 것과 동일한 항목), 리본 버튼이 뜬다. `uninstall-excel-addin`으로 제거.
**클라우드·인터넷 전혀 불필요 - 폐쇄망 배포는 이 명령 하나(또는 그 레지스트리
키를 GPO/스크립트로 푸시).** 버튼을 누르면 §10의 office.js 없는 순수 웹
패널이 태스크팬으로 열린다(인증서가 신뢰돼 있으니 WebView2가 로컬 HTTPS를
그대로 렌더).

**버튼을 눌러 패널이 실제로 열리는 것까지 시각 확인(2026-08-17 검증 완결)**:
처음엔 원격 SSH가 세션 1 네이티브 UI와 격리돼 픽셀 좌표 클릭이 리본 버튼에
안 닿았다(사용자 화면의 다른 창이 겹침 + foreground 락). 픽셀 사냥 대신 UI
Automation `InvokePattern`으로 리본 버튼을 이름("xgen-seepage 채팅")으로 찾아
직접 Invoke하니 **태스크팬이 Excel 오른쪽에 실제로 도킹되며 열렸다**(스크린샷
확인: Excel Y·Z 열 옆에 도킹, 제목 "xgen-seepage", 초록 상태바 "XGEN 연결됨",
"에이전트" 드롭다운에 실제 계정 워크플로우, "모델" 드롭다운). office.js 없는
순수 웹 UI가 태스크팬 안에서 그대로 렌더됐고 "Office.js 로딩중" 멈춤은 없었다.
브라우저로 여는 `xgen-seepage panel`도 이미 완전 검증됨.

**그 구멍을 코드로 메웠다 - `xgen-seepage panel`**: office.js를 걷어내면서
채팅 패널이 순수 웹 UI가 됐으므로(§10), Excel 리본 버튼(Office 애드인)이
이 Office 버전에서 안 떠도 패널을 브라우저로 그냥 열면 그대로 쓸 수 있다.
`xgen-seepage panel` 명령(또는 `run --open-panel`)이 `https://127.0.0.1:
<port>/index.html`을 기본 브라우저로 연다. `run`도 시작 시 이 URL을
명확히 출력한다. 즉 리본 버튼 surfacing은 Office 버전에 달렸지만, **채팅
패널 사용 자체는 어느 머신에서든 보장된다** - 이게 "남은 것 없이" 닫는
현실적 방법이다. Office 애드인 매니페스트(리본 버튼)는 지원되는 Office
에서 쓰라고 그대로 남겨뒀다.

**패널 안에서 에이전트 선택(토글) + 폐쇄망 대응(2026-08-17 사용자 요청)**:
태스크팬을 워크플로우 하나에 고정하지 않고, XGEN에 로그인만 돼 있으면 그
계정이 가진 워크플로우(에이전트) 중 하나를 패널 드롭다운에서 골라 바로
...
**패널이 실제로 쓰는 경로(`/chat/stream`)로 "채팅 → 진짜 셀 편집"을 별도
프로세스에서 read-back으로 다시 닫았다(2026-08-17)**: 세션 1에 A1="BEFORE_CHAT"
인 워크북을 열어두고, 별도로 도는 커넥터에 패널과 똑같은 `POST /chat/stream`
(provider=anthropic 주입, workflow=`shinhan_blue_noRAG_tmp`)으로 "A1을 42로 바꿔"를
보냈다. 로그에 커넥터 도구 24개가 에이전트에 자동 주입된 게 찍혔고, 에이전트가
`list_open_workbooks`로 워크북을 찾아 `set_live_cell`로 편집 → **A1 read-back = 42.0**.
에이전트 응답도 "셀을 42로 변경했습니다. 이전 값은 'BEFORE_CHAT'이었습니다"로
이전 값을 정확히 알고 있었다(= 라이브 셀을 실제로 읽었다는 증거). 이로써 리본
버튼 → 도킹 패널 → 패널 채팅 → XGEN 에이전트 → 커넥터 도구 → 실제 Excel 셀
편집까지 전 경로가 실기로 닫혔다.

**이 과정에서 배포용 함정 하나를 실측으로 규명했다 - 커넥터는 Excel과 같은
Windows 세션에서 돌아야 한다**: 커넥터를 서비스 세션 0에서, Excel을 콘솔 세션 1
에서 띄우면 COM이 교차 세션 접근을 못 해 `list_open_workbooks`가 빈 목록을
반환하고 에이전트가 "열린 통합문서가 없다"고 답한다(셀은 안 바뀜). 커넥터를
Excel과 같은 세션 1에서 다시 띄우니 즉시 위 결과가 나왔다. 정상 사용(사용자가
자기 데스크톱 세션에서 `xgen-seepage run`을 켜고 같은 세션의 Excel을 씀)에서는
자동으로 충족되지만, 서비스·원격 실행으로 배포할 땐 **대화형 데스크톱 세션에서
돌려야 한다**(§12).
...
벤더링하는 선에서 그쳤는데, 사용자가 "Office 비의존이 아니라 XGEN 의존으로
가야 한다"고 정확히 지적했다. 맞는 지적이었다 - §10에 적었듯 이 패널은
Office JS API를 하나도 안 쓰므로 office.js 자체가 불필요한 짐이었고,
오히려 `Office.onReady()`가 Office 호스트를 기다리며 UI를 멈추게 하고
있었다. office.js `<script>`와 `Office.onReady()` 게이팅을 전부 걷어내고,
DOM 로드 시점에 `/health`로 커넥터 연결만 확인해 바로 뜨도록 했다. 실측
(Playwright): 상태가 "Office.js 로딩중" 멈춤에서 **"XGEN 연결됨"**으로
바뀌고, 페이지 로드 시 **외부(Microsoft 포함) 요청이 0건**이며, 에이전트
드롭다운·채팅 전송이 정상 동작한다. 이제 office.js·에어갭 관련 미검증
항목 자체가 사라졌다(안 쓰니까). 매니페스트(리본 버튼용)는 남겨뒀지만 그
경로로 태스크팬을 띄워도 페이지는 office.js 없이 그냥 웹 페이지로 렌더된다.

**핵심(이제 UI까지 포함해 전부 인터넷 0)**: 에이전트가 Excel에 침투해 셀을
읽고/쓰는 본체(로그인 → WS 브릿지 → tools → live_adapter/libreoffice_adapter)도,
그 위의 채팅 패널 UI도 이제 전부 XGEN 커넥터에만 의존한다. XGEN 서버가
폐쇄망 안에 있으면 모든 통신이 폐쇄망 안에서만 오간다(§4·§5). Microsoft
애드인 인프라도, office.js도, 인터넷도 어디에도 필요 없다.
...
- **커넥터는 Excel과 같은 Windows 세션에서 돌아야 한다(§11 실측)**: `live_adapter`
  의 COM 접근은 교차 세션이 안 된다. 커넥터를 서비스 세션 0에서 돌리고 Excel이
  콘솔 세션 1에 있으면 `list_open_workbooks`가 빈 목록을 반환한다(에이전트가
  "열린 통합문서가 없다"고 답함). 사용자가 자기 데스크톱에서 `xgen-seepage run`을
  켜는 정상 사용에선 자동 충족되지만, 서비스/스케줄러/원격으로 배포할 땐 대화형
  데스크톱 세션(콘솔 로그온)에서 실행하도록 배포 문서에 명시해야 한다.
...
## 13. 서버 전환(jeju/dev/prod) + 403 Forbidden (2026-08-17)

여러 XGEN을 오갈 때 실사용에서 나온 문제("포비든 떠서 못 쓴다")를 파고들어,
xgen-connector의 서버/인증 처리를 참고(`src/main/index.ts`의 서버 전환,
`docs/PROTOCOL.md`의 "게이트웨이가 JWT로 X-User-* 주입·권한 판정")해 정리했다.

- **403은 토큰이 아니라 권한 문제다.** 게이트웨이는 `Authorization: Bearer`만
  받고 권한을 JWT로 판정한다(클라이언트가 X-User-* 같은 신원 헤더를 보내면
  안 된다 - 게이트웨이가 붙인다). 그래서 401은 재로그인으로 풀리지만 403은
  안 풀린다. 실측: dev-xgen에서 개인 계정은 superuser라 모든 엔드포인트가
  200이었다 - 즉 사용자의 403은 **그 계정이 superuser가 아닌 다른 서버**에서
  워크플로우 권한(`main.agentflow:read`류)이 없어서 난 것이다. 그래서 코드로
  "고칠" 버그가 아니라, (a) 403을 실행 가능한 안내로 바꾸고 (b) 권한 있는
  서버·계정으로 쉽게 옮기게 하는 게 맞는 대응이다.
- **토큰을 서버별로 저장한다.** 예전엔 키체인 단일 슬롯이라, 서버 B로 바꿔도
  A의 토큰이 남아 B에 A 토큰을 보내 403/401이 날 수 있었다(로그인이 config는
  B로 바꾸고 토큰 저장이 실패하면 특히). 키를 `access_token::<server_url>`로
  나눠 그 불일치 자체를 없앴다(예전 단일 슬롯은 읽기 폴백으로 자동 마이그레이션).
  로그인도 토큰을 먼저 저장하고 성공해야 활성 서버를 바꾼다.
- **`xgen-seepage server list`/`use`.** 로그인해 본 서버와 각 서버의 토큰 보유
  여부를 보고, 토큰이 있으면 재로그인 없이 활성 서버를 전환한다(전환 시 그
  서버 기준 user_id/username으로 갱신 - 서버마다 사용자 식별자가 다르므로
  브릿지 ws url에 옛 user_id를 쓰면 안 된다). xgen-connector는 서버를 바꾸면
  토큰을 통째로 지우고 재로그인시키는데, 서버별 토큰으로 그 재타이핑을 없앴다.
- **패널·`status`가 지금 붙은 서버·계정과 권한을 보여준다.** 패널 헤더에
  `GET /server`로 "서버 · 사용자"를 띄우고(엉뚱한 서버에 붙은 걸 눈으로 잡음),
  403이 나면 "이 계정은 이 서버에서 권한이 없다 - 서버/계정을 확인하라"고
  안내한다. `status`는 superuser/권한을 보고 agentflow 권한이 없으면 미리 경고.
- **한계**: 활성 서버 전환은 `run` 프로세스가 시작할 때의 서버에만 바인딩된다
  (브릿지 ws·패널 프록시 모두 startup의 server_url을 쓴다). 서버를 바꾼 뒤엔
  `run`을 재시작해야 반영된다 - 런타임 핫스왑은 브릿지 재접속·프록시 base_url
  교체까지 얽혀 위험 대비 이득이 적어, 재시작 필요를 CLI가 명시하는 선으로 뒀다.
