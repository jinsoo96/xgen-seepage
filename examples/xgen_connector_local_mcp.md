# xgen-connector의 "로컬 MCP"에 xgen-seepage 등록하기

`xgen-connector`(PlateerLab, 데스크톱 XGEN 클라이언트)는 이미 로컬 MCP 서버를
XGEN 에이전트 세션에 자동으로 연결해 주는 브릿지를 갖고 있다. xgen-seepage는
그 위에 등록만 하면 된다 — 별도 워크플로우 편집이나 코드 수정이 필요 없다.
