...
`maybe_number`/`cell_text`의 판정 규칙은 다음 이유로 정했다. COM으로 셀에
문자열을 그대로 쓰면 Excel의 "타이핑하면 자동으로 숫자가 되는" 동작이
일어나지 않아 금액 셀이 문자로 굳어 SUM 등의 수식이 깨진다. 반면
전화번호·우편번호·사번처럼 선행 0이 있거나 대시가 섞인 값을 숫자로 바꾸면
의미가 훼손된다. 편집 도구 전반이 같은 휴리스틱을 쓰면 LLM 입장에서
동작이 예측 가능해진다.
"""
